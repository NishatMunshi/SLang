...
ast_node_un_expr *parser_parse_un_expr(parser *parser) {
    ...
    switch (token_get_type(current_token)) {
        case TOKEN_NUM:
            return ast_node_un_expr_create_num(parser_parse_num(parser));
        case TOKEN_IDENT:
        ...
        case TOKEN_LPAREN:
            return ast_node_un_expr_create_paren_expr(parser_parse_paren_expr(parser));

        default:
            PARSER_ERROR(current_token, "an unary expression");
    }
}