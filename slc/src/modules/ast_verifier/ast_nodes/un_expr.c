...
void ast_verifier_verify_un_expr(ast_verifier *ast_verifier, ast_node_un_expr *node_un_expr) {
    switch (ast_node_un_expr_get_type(node_un_expr)) {
        ...
        case AST_NODE_UN_EXPR_PAREN_EXPR:
            ast_verifier_verify_expr(ast_verifier, ast_node_paren_expr_get_expr(ast_node_un_expr_get_paren_expr(node_un_expr)));
            break;
        case AST_NODE_UN_EXPR_CALL:
            ast_verifier_verify_call(ast_verifier, ast_node_un_expr_get_call(node_un_expr));
            break;
