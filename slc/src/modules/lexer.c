...
static void lexer_init(string_view *source_code) {
...
static token *lexer_lex_operator() {
...
static token *lexer_lex_unidentified_char() {
    size_t begin = lexer.current_index;
    size_t line = lexer.line;
    size_t col = lexer.col;
    lexer_advance(lexer);
...
static void lexer_chop_whitespace() {
